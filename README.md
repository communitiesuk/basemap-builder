...
We want to reduce the extent of the basemap to only include areas within England. 

Go to the [GeoPortal](https://geoportal.statistics.gov.uk/), and in the menu, select Boundaries > Administrative Boundaries > Countries > [Latest Year] Boundaries, select the BFC option. 

Use the filters to select only England (via `CTRYXXNM` or `CTRYXXCD`), and download the GeoPackage file (make sure to select 'Toggle Filters' to include only England).

## 4. Clip the layers to England and output to single files in a directory
We will use the `clip_parallel.sh` file to clip the selected layers to just the features that fit within the England boundary. 

Check that the two GeoPackage files you want to use are named in this script, and that they are in the same directory as the script. 
